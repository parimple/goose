...
pub async fn docx_tool(
    ...
    match operation {
                ...
                UpdateMode::Replace { old_text } => {
                    ...
                    if !found_text {
                        return Err(ErrorData {
                            code: ErrorCode::INTERNAL_ERROR,
                            message: Cow::from(format!("Could not find text to replace: {}", old_text)),
                            data: None,
                        });
                    }
                    ...
                    let image_data = if extension != "png" {
                        ...
                        img.write_to(&mut Cursor::new(&mut png_data), ImageFormat::Png)
                            .map_err(|e| {
                                ErrorData {
                                    code: ErrorCode::INTERNAL_ERROR,
                                    message: Cow::from(format!("Failed to convert image to PNG: {}", e)),
                                    data: None,
                                }
                            })?;
                        png_data
        ...
        _ => Err(ErrorData {
                code: ErrorCode::INVALID_PARAMS,
                message: Cow::from(format!("Invalid operation: {}. Valid operations are: 'extract_text', 'update_doc'", operation)),
                data: None,
            })
    }
}
